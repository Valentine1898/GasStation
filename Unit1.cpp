...
{
ADOQuery2->Close();
ADOQuery2->SQL->Text="SELECT АЗС.Назва \
FROM АЗС;                \
";
ADOQuery2->Open();
}
...
{
ADOQuery3->Close();
ADOQuery3->SQL->Text="SELECT Паливо.Назва \
FROM Паливо;                \
";
ADOQuery3->Open();
}
...
if(DBLookupComboBox2->Text == "А-92")
        a = StrToFloat(DBEdit2->Text);

else {
if(DBLookupComboBox2->Text == "А-95")
        a = StrToFloat(DBEdit3->Text);

else {
if(DBLookupComboBox2->Text == "А-98")
        a = StrToFloat(DBEdit4->Text);

else {
if(DBLookupComboBox2->Text == "ДП")
        a = StrToFloat(DBEdit5->Text);

else{
if(DBLookupComboBox2->Text == "Газ")
...
{
ShowMessage("Недостатньо палива в бензоколонці, вибачте за тимчасові незоручності");
Form1->ADOQuery4->SQL->Clear();
Form1->ADOQuery4->SQL->Add("INSERT INTO продажі ([АЗС],[Паливо],[Кількість],[Вид_оплати],[Дата]) VALUES (:text, :text1, :text2 , :text3, :text4)");
...
Form1->ADOQuery4->Parameters->ParamByName("text3")->Value = "Готівка";
...
ADOQuery15->Close();
ADOQuery15->SQL->Text="UPDATE Паливо SET Паливо.Ціна = [Ціна]*:param;";
